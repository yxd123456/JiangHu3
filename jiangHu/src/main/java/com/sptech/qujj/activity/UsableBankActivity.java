...
public class UsableBankActivity extends BaseActivity implements OnClickTitleListener, OnClickListener {
	private TitleBar titleBar;
	private ListView listview_card; // 银行卡列表
	private UsableCardAdapter lvBankCardAdapter;
	...
	private void initView() {
		...
		blist = (List<UsablebankBean>) getIntent().getSerializableExtra("blist");

		spf = getSharedPreferences(Constant.USER_INFO, Context.MODE_PRIVATE);
		listview_card = (ListView) findViewById(R.id.listview_card);
		lvBankCardAdapter = new UsableCardAdapter(UsableBankActivity.this, blist);
	...
	public void test(int position) {
		...
		setResult(AddBankcardActivity.resultOk, intent);
		// finish();
		ActivityJumpManager.finishActivity(this, 1);
	}
