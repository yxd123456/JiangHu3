...
public class DialogAlert1 {
	...
	private String hour = "06", min = "30";
	EventAlertListener eventAlerteListener;
	private PickerView minute_pv;
	private PickerView second_pv;
	private int hh, mm;

	public DialogAlert1(Context context, int hh, int mm, EventAlertListener eventAlerteListener) {
	...
	public void createMyDialog() {
		View view = LayoutInflater.from(context).inflate(R.layout.dialog_view_alert1, null);
		if (hh < 10) {
		...
		dialog_im_right.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				closeDialog();
				String data = hour + ":" + min;
				int miao = ((Integer.valueOf(min)) * 60) + ((Integer.valueOf(hour)) * 60 * 60);
