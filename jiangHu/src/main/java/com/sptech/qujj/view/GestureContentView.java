...
import com.sptech.qujj.model.GesturePoint;
...
public class GestureContentView extends ViewGroup {
	...
	public GestureContentView(Context context, boolean isVerify, String passWord, GestureCallBack callBack) {
		...
		width = screenDispaly[0];
		blockWidth = width * 10 / 35;
	...
	public void setParentView(ViewGroup parent) {
		...
		LayoutParams layoutParams = new LayoutParams(width - width * 10 / 72, width - width * 10 / 72);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		for (int i = 0; i < getChildCount(); i++) {
			...
			int col = i % 3;
			View v = getChildAt(i);
			v.layout(col * blockWidth + blockWidth / baseNum, row * blockWidth + blockWidth / baseNum, col * blockWidth + blockWidth - blockWidth / baseNum, row * blockWidth + blockWidth - blockWidth
					/ baseNum);
		}
	}
