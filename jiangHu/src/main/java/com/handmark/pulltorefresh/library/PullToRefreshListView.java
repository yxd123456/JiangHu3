...
public class PullToRefreshListView extends PullToRefreshAdapterViewBase<ListView> {
	...
	protected boolean isReadyForPullStart() {
		...
		if (mRefreshableView.getScrollY() == 0) {
			// 2015年3月29日12:07:47 加上如果没有数据也可以下拉刷新
			if (v == null) {
				return true;
			}
			if (v != null) {
	...
	protected boolean isReadyForPullEnd() {
		...
		if (null == mRefreshableView.getAdapter()) {
			result = false;
		} else {
			int itemCount = mRefreshableView.getAdapter().getCount();
	...
	protected class InternalListView extends ListView implements EmptyViewMethodAccessor {
		...
		protected void dispatchDraw(Canvas canvas) {
			...
			/*
			 * modify by max 2014-03-24 begin
			 * 解决java.lang.IllegalArgumentException: bitmap size exceeds 32bits
			 */
			catch (Exception e) {
				e.printStackTrace();
			//	Logger.e("PullToMoreListView", e.getMessage());
			}
			/* modify by max 2014-03-24 end */
		}
		...
		/* add by max 2014-03-24 begin */
		// 解决这个异常：java.lang.IllegalStateException: The content of the adapter
		...
		// but only from the UI thread. [in ListView(-1, class
		// com.echoliv.upairs.custom.PullToRefreshListView) with Adapter(class
		// android.widget.HeaderViewListAdapter)]
		// I have pageable ListView and its rows are being loaded asynchronously
		// while scrolling down and sometimes the app just crashes. It's not
		// happening on Android 2.3.3 but does happen on Android 4.0 and above.
		// 连接地址：http://stackoverflow.com/questions/15273956/android-listview-with-paging-crashes(遇到了一模一样的问题)
		@Override
		protected void layoutChildren() {
			//Logger.d("show", "layoutChildren()");
			try {
				super.layoutChildren();
			} catch (IllegalStateException e) {
			//	Logger.e("InternalListView", "This is not realy dangerous problem");// 这是一个不需要处理的错误。
			}
		}
		/* add by max 2014-03-24 end */
	}
}
